  ...
  ({ open, onOpenChange, phone = '', courierHistory: externalHistory, onCheckPhone, isChecking }, ref) => {
    ...
    const { data: fetchedHistory = [], refetch, isRefetching } = useQuery({
      ...
      queryFn: async () => {
        ...
        const { data, error } = await supabase.from('customer_courier_history').select('*').eq('phone', normalizedInputPhone);
    ...
    useEffect(() => {
      if (!isChecking && lastCheckedPhone) {
        const timer = setTimeout(() => { refetch(); queryClient.invalidateQueries({ queryKey: ['courier-history'] }); }, 600);
        setLastCheckedPhone(null);
        return () => clearTimeout(timer);
      }
    }, [isChecking, lastCheckedPhone, refetch, queryClient]);

    const handleCheck = async () => { const normalized = normalizePhone(inputPhone); if (normalized.length >= 11) { setLastCheckedPhone(normalized); await onCheckPhone(normalized); } };
    ...
    const lastChecked = courierHistory.length > 0 ? courierHistory.reduce((latest, h) => new Date(h.checked_at) > new Date(latest) ? h.checked_at : latest, courierHistory[0].checked_at) : null;
        ...
        <ResponsiveModalContent ref={ref} className="max-w-md bg-gradient-to-br from-slate-900 to-slate-800 text-white border-slate-700" drawerClassName="bg-gradient-to-br from-slate-900 to-slate-800 text-white border-slate-700">
          <ResponsiveModalHeader>
            <ResponsiveModalTitle className="text-center">
              <span className="bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent text-xl font-bold">Fraud Check</span>
              <p className="text-sm text-slate-400 font-normal mt-1">Check delivery history</p>
            </ResponsiveModalTitle>
          </ResponsiveModalHeader>
          <div className="space-y-4">
            <div className="relative">
              <Input value={inputPhone} onChange={(e) => setInputPhone(e.target.value)} placeholder="Phone number..." className="bg-slate-900/60 border-slate-700 text-white placeholder:text-slate-500 pr-12" />
            ...
            {courierHistory.length > 0 && (
              <>
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-white/5 rounded-xl p-3 text-center"><p className="text-2xl font-bold text-white">{stats.totalOrders}</p><p className="text-xs text-slate-400">Total</p></div>
                  <div className="bg-white/5 rounded-xl p-3 text-center"><p className="text-2xl font-bold text-emerald-400">{stats.totalDelivered}</p><p className="text-xs text-slate-400">Delivered</p></div>
                  <div className="bg-white/5 rounded-xl p-3 text-center"><p className="text-2xl font-bold text-red-400">{stats.totalCancelled}</p><p className="text-xs text-slate-400">Cancelled</p></div>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Success Rate</span>
                    <span className={cn('font-bold', stats.successRate > 70 ? 'text-emerald-400' : stats.successRate >= 50 ? 'text-yellow-400' : 'text-red-400')}>{stats.successRate.toFixed(1)}%</span>
                  </div>
                ...
                <div className="space-y-3">
                  <h3 className="text-sm text-slate-400 border-b border-slate-700 pb-2">Courier Details</h3>
                ...
                {lastChecked && <p className="text-xs text-slate-500 text-center">Last checked: {formatRelativeTime(lastChecked)}</p>}
              </>
            )}
            {courierHistory.length === 0 && !showLoading && (<div className="text-center py-6 text-slate-400"><p>No history found</p></div>)}
            {showLoading && (<div className="text-center py-6"><Loader2 className="h-8 w-8 animate-spin text-indigo-400 mx-auto" /><p className="text-sm text-slate-400 mt-2">Checking...</p></div>)}
          </div>
