...
export function SendToCourierModal({ open, onOpenChange, orderId, onSuccess }: SendToCourierModalProps) {
  ...
  const handleSubmit = async () => {
    const result = await sendToCourier({ orderId, provider, cityId: cityId ? parseInt(cityId) : undefined, zoneId: zoneId ? parseInt(zoneId) : undefined, areaId: areaId ? parseInt(areaId) : undefined, weight: parseFloat(weight), specialInstructions });
      ...
      <ResponsiveModalContent className="sm:max-w-[425px]">
        <ResponsiveModalHeader>
          <ResponsiveModalTitle className="flex items-center gap-2">
            <Truck className="h-5 w-5" /> কুরিয়ারে পাঠান
          </ResponsiveModalTitle>
          <ResponsiveModalDescription>প্রোভাইডার নির্বাচন করুন</ResponsiveModalDescription>
        </ResponsiveModalHeader>
        ...
        <div className="space-y-4 py-2">
            ...
            <>
              <div className="space-y-2">
                ...
                <Select value={cityId} onValueChange={handleCityChange} disabled={isLoadingCities}>
                  <SelectTrigger><SelectValue placeholder={isLoadingCities ? 'Loading...' : 'Select city'} /></SelectTrigger>
                  <SelectContent>{cities.map((city) => <SelectItem key={city.city_id} value={city.city_id.toString()}>{city.city_name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label>Zone *</Label>
                <Select value={zoneId} onValueChange={handleZoneChange} disabled={!cityId || isLoadingZones}>
                  ...
                  <SelectContent>{zones.map((zone) => <SelectItem key={zone.zone_id} value={zone.zone_id.toString()}>{zone.zone_name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label>Area (Optional)</Label>
                <Select value={areaId} onValueChange={setAreaId} disabled={!zoneId || isLoadingAreas}>
                  <SelectTrigger><SelectValue placeholder={isLoadingAreas ? 'Loading...' : 'Select area'} /></SelectTrigger>
                  <SelectContent>{areas.map((area) => <SelectItem key={area.area_id} value={area.area_id.toString()}>{area.area_name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea value={specialInstructions} onChange={(e) => setSpecialInstructions(e.target.value)} placeholder="কুরিয়ারের জন্য নোট..." rows={2} />
          </div>
        </div>
        ...
        <ResponsiveModalFooter>
          ...
          <Button onClick={handleSubmit} disabled={isSending || !canSubmit}>
            {isSending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} পাঠান
          </Button>
        </ResponsiveModalFooter>
      </ResponsiveModalContent>
    </ResponsiveModal>
